...
**Naming rule established during Phase 1 build:** an app earns the `-orchestrator` suffix only if it consumes an outcome it did not produce and routes to different commands, against different bounded contexts, based on that outcome. An app with many handlers or complex internal logic is not automatically an orchestrator; `inventory` has both `reserve` and `release` operations but never decides between them itself, it only executes whichever command it's given. This is why `payment-orchestrator`, as named in the original blueprint, is a naming error corrected in this build: it manages Stripe's own call lifecycle (auth, capture, void, refund) via direct, undecided command execution, and does not branch on an outcome it didn't produce. It should be named `payment`, plain, matching `inventory`.
